{
  ...
  owner = "153639664572563456";

  dataPath = "/var/lib/jmusicbot";

  configFile = pkgs.writeText "jmusicbot-config" ''
    /////////////////////////////////////////////////////////
  ...
  '';
in {
  # https://discord.com/api/oauth2/authorize?client_id=1006822560471711765&permissions=0&scope=bot
  # environment.systemPackages = with pkgs; [
  #   jmusicbot
  # ];

  age.secrets.discord-jmusicbot-token.file = ../../secrets/discord-jmusicbot-token.age;

  systemd.services.jmusicbot = {
    description = "jmusicbot";
    preStart = ''
      mkdir -p ${dataPath}
      cp "${configFile}" "/var/lib/jmusicbot/config.txt"
    '';
    script = ''
      cd ${dataPath}
      export TOKEN=$(cat ${config.age.secrets.discord-jmusicbot-token.path})
      ${pkgs.jmusicbot}/bin/JMusicBot
    '';
    # serviceConfig = {
    #   Restart = "on-failure";
    #   RestartSec = "5m";
